...
<body>
 ...
 <h1>BANCO DE ORCAMENTOS</h1>
        <table>
            ...
            <tbody>
        <%
        ArrayList<Cidade> cidades = CidadeBD.listar();
        
        for(int i=0; i < cidades.size(); i++){
            Cidade cadaCidade = cidades.get(i);
        %>
        <tr>
            <td><%=cadaCidade.getNome()%></td>
            <td><%=cadaCidade.getEmail()%></td>
            <td><%=cadaCidade.getOrigem()%></td>
            <td><%=cadaCidade.getDestino()%></td>
            <td>
                <a href="excluir.jsp?codigo=<%=cadaCidade.getCodigo()%>" onclick="return confirm('Deseja realmente excluir ?')">Excluir</a>
            </td>
        </tr>
        <%
        }
